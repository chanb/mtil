...
from jaxl.buffers.buffers import (
    ReplayBuffer,
    CheckpointIndexError,
    NoSampleError,
)
import jaxl.constants.buffers as c


"""
Default replay buffers.
- NextStateNumPyBuffer is the standard implementation that uses a cyclic buffer.
XXX: Try not to modify the above buffers.
XXX: Feel free to add new buffer types as needed (e.g. using PyTree instead).
XXX: Extensions such as prioritized experience replay and hindsight experience replay
     should be a wrapper.
"""
...
class TransitionNumPyBuffer(AbstractNumPyBuffer):
    ...
    def sample_with_next_obs(
        self,
        batch_size: int,
        idxes: Optional[chex.Array] = None,
        **kwargs,
    ) -> Tuple[
        chex.Array,
        chex.Array,
